...
static int connection, domain, error_code, calls, state = 5, connections, domains, autostart;
static bool fail_write, fail_action, fail_info, missing;
static const char* uuid = "11111111-2222-3333-4444-555555555555";
extern "C" {
void fakeReset() { state = 5; autostart = 0; error_code = calls = 0; fail_write = fail_action = fail_info = missing = false; }
...
int virDomainGetAutostart(void*, int* value) { *value = autostart; return 0; }
...
int virDomainSetAutostart(void*, int value) {
    ++calls;
    if (fail_action) { error_code = 1; return -1; }
    autostart = value ? 1 : 0;
    return 0;
